...
The trainer (`scripts/01_train_dense_reconstruction.py`) interleaves streaming datasets by
...
`scripts/02_train_dense_sft.py` on **`nvidia/OpenCodeInstruct`** (rendered to a JSONL of chat
...
`scripts/sft_kernel.py` on **`SakanaAI/AI-CUDA-Engineer-Archive`**, splits `level_1,level_2`,
...
`scripts/dpo_sakana.py` mines the Sakana archive's evolutionary trajectory: per `Task_ID`,
